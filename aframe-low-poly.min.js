class LowPoly {
    static addMappings(name, mapping) {
        return Object.assign({}, mapping, { amplitude: name + ".amplitude", "amplitude-variance": name + ".amplitudeVariance", seed: name + ".seed" });
    }
    static addSchema(schema) {
        return Object.assign({}, schema, { amplitude: { default: 0.05 }, amplitudeVariance: { default: 0.001 }, flatShading: { default: false }, seed: { default: "apples" } });
    }
...
class LowPolyFactory {
    static simple(geometryName, createGeometry, properties) {
        ...
        AFRAME.registerPrimitive("lp-" + geometryName, extendDeep({}, meshMixin, { defaultComponents: defaultComponents, mappings: LowPoly.addMappings(componentName, primitiveMapping) }));
...
class Random {
    ...
    static xfnv1a(k) {
        for (var i = 0, h = 2166136261 >>> 0; i < k.length; i++) h = Math.imul(h ^ k.charCodeAt(i), 16777619);
        return function () {
            h += h << 13;
            h ^= h >>> 7;
            h += h << 3;
...
LowPolyFactory.simple("cone", createCylinderGeometry, [
    { hyphenated: "radius-top", camelCased: "topRadius", schemaValue: { default: 100 } },
...
function createCylinderGeometry(data) {
    return new THREE.CylinderGeometry(data.topRadius, data.bottomRadius, data.height, data.radialSegments, data.heightSegments);
}
